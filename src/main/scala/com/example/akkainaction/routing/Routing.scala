...
class RedirectActor(pipe: ActorRef) extends Actor with ActorLogging {
  log.info("RedirectActor instance created")
...
class SpeedRouterLogic(minSpeed: Int, normalFlowPath: String, cleanUpPath: String) extends RoutingLogic {
  def select(message: Any, routees: immutable.IndexedSeq[Routee]): Routee = {
    message match {
      case msg: Photo =>
        if (msg.speed > minSpeed) findRoutee(routees, normalFlowPath) else findRoutee(routees, cleanUpPath)
    }
  }

  private def findRoutee(routees: immutable.IndexedSeq[Routee], path: String): Routee = {
